...
class Trainer:
    ...
    def _setup_transforms(self):
        ...
        self.train_transform = transforms.Compose([
            ...
            transforms.RandomRotation(
                degrees=(-15, 15),  # Controlled rotation
                fill=(0,)  # Fill with black
            ),
            transforms.RandomAffine(
                degrees=0,
                translate=(0.1, 0.1),  # Random translation up to 10%
                scale=(0.85, 1.15),  # Random scaling ±15%
                fill=(0,)
            ),
            transforms.ColorJitter(
                brightness=0.2,  # Adjust brightness
                contrast=0.2,  # Adjust contrast
                saturation=0.0,  # No saturation change for X-rays
                hue=0.0  # No hue change for X-rays
            ),
            transforms.GaussianBlur(
                kernel_size=3,  # Simulate different X-ray qualities
                sigma=(0.1, 2.0)
            ),
            ...
            transforms.RandomErasing(
                p=0.2,  # 20% chance to apply
                scale=(0.02, 0.1),  # Size of erasing
                ratio=(0.3, 3.3),
                value=0  # Fill with black
            )
        ])

        # Validation transform remains the same
    ...
    def plot_training_progress(self):
        ...
        plt.tight_layout()
        plt.savefig('training_progress2.png', dpi=300, bbox_inches='tight')
        plt.close()
